...
function loadSimpleFilter(blocks, space) {
  // If the target is a PIC16-based controller, the application code and
  // EEPROM contents are combined in a single memory space furnished by the bootloader,
  // and consequently they are also combined in the .hex file.
  // 
  // The EEPROM's limits are given inclusively in terms of blocks within the space,
  // space.combinedEEBlockStart <= EEPROM blocks <= space.combinedEEBlockEnd.
  //
  // If this is the case (space.combinedEE == true) and writing the EE blocks
  // is not enabled (space.combinedEEWritable != true), we filter out those
  // blocks. 
  //
  ...
  if ((space.combinedEE) &&
      (!space.combinedEEWritable)) {
    for (let blockIndex = space.combinedEEBlockStart;
         blockIndex <= space.combinedEEBlockEnd;
         blockIndex++) {
      console.log("loadSimpleFilter: Removing EE block", blockIndex);
...
class PIC16TinyBL {
  constructor( options ) {
    ...
    this.combinedEE = options.combinedEE || true,
    this.combinedEEBlockStart = options.combinedEEBlockStart || 0x780;
    this.combinedEEBlockEnd = options.combinedEEBlockEnd || 0x787;
    this.combinedEEWritable = false;
